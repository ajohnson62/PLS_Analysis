...
use_mask <- FALSE
mask <- data.frame(read.csv(paste(wd, freq_range, '/', 'groupEnvPartialCorrelationRegularized_z.csv', sep='' ), header=FALSE))
z_thresh <- 3.7585
...
for(contrast in c(4)){
  ...
  hold_fit <- NULL
  x_orig <-y_orig<- x_hold <- y_hold <- NULL
  ncomp <-1
  comp <- 1
  varselect_perm <- 100
  nperm = 1000
  ...
  for(cv in 1:10){
    ...
    for(imp in 1:5){
      ...
      # Keep aside 10% of the data for hold out
      y_hold[[imp]] <- y_orig[[imp]][holdout[[cv]],]
      x_hold[[imp]] <- x_orig[[imp]][holdout[[cv]],]
      y_orig[[imp]] <- y_orig[[imp]][-holdout[[cv]],]
      x_orig[[imp]] <- x_orig[[imp]][-holdout[[cv]],]
    }
    ...
    for(j in 1:varselect_perm){ #repeat several times (e.g. 100)
      ...
      for(imp in 1:5){
        ...
        CI_upper <- list()
        k <- 1
        #keepX <- round(seq(2,ncol(x_orig[[imp]]),(ncol(x_orig[[imp]])-2)/40),0) #Sequence of tuning parameter
        keepX <- exp(seq(log(10), log(filter), length.out = 20))
        for(i in keepX){
          # #Filter the data to include top correlations
          # df <- data.frame(names=character(),av_pval = double(),stringsAsFactors=FALSE) 
          # pval_lm <- data.frame(pval_lm=double())
          # for(x in 1:ncol(x_train[[imp]])){
          #   data <- x_train[[imp]][,x]
          #   for(y in 1:ncol(y_train[[imp]])){
          #     fit = lm(data ~ y_train[[imp]][,y])
          #     pval_lm[y,1] <-  anova(fit)$'Pr(>F)'[1] 
          #   }
          #   df[x,] <- c(colnames(x_train[[imp]])[x], min(pval_lm))
          # }
          # x_train_filter[[imp]] <- x_train[[imp]]
          # x_train_filter[[imp]][,df$names[order(df$av_pval, decreasing = FALSE)][1:(ncol(x_train[[imp]])-filter)]] <- rep(NA, nrow(x_train[[imp]]))
          # 
          train_cov <- apply(cov(x_train[[imp]], y_train[[imp]]), 1, max)
          x_train_filter[[imp]] <- x_train[[imp]]
          x_train_filter[[imp]][,names(train_cov[order(abs(train_cov), decreasing = FALSE)])[1:(ncol(x_train[[imp]])-filter)]] <- rep(NA, nrow(x_train[[imp]]))
          train_fit <- spls(x_train_filter[[imp]], y_train[[imp]],  mode = "canonical", ncomp =comp, scale=TRUE,  keepX=i)
    ...
    fname <- paste(wd,'/', A, '_', B,'_cv_', cv,  '_filter_', filter,'_component_', comp, '_select_x_train_and_test.png', sep='' )
    ggsave(fname, width = 10, height = 10, units = "cm")
    fname <- paste(wd,'/', A, '_', B,'_cv_', cv,  '_filter_', filter,'_component_', comp, '_select_x_train_and_test.csv', sep='' )
    ...
    ggplot(df2)+
      ...
      theme_minimal()+
      ylab('Mean covariance')
    fname <- paste(wd,'/', A, '_', B,'_cv_', cv,  '_filter_', filter,'_component_', comp, '_select_x_test.png', sep='' )
    ggsave(fname, width = 10, height = 10, units = "cm")
    ...
    ggplot(data_melt, aes(x=Var2, y=covariance, fill=Var2)) + 
      ...
      theme_minimal()+
      guides(fill=FALSE)
    fname <- paste(wd,'/', A, '_', B,'_cv_', cv,  '_filter_', filter,'_', comp, 'select_x_test_box_plot.png', sep='' )
    ...
    signs_Y <- NULL
    for(imp in 1:5){
      #Fit PLS model with chosen keepX and project onto holdout dataset to get covariance
      preProc  <- preProcess(x_orig[[imp]], method=c("center", "scale")) #This ensures that the hold out data is scaled using the tranformation used for the training set
      x_orig[[imp]] <- predict(preProc, x_orig[[imp]])
      x_hold[[imp]]  <- predict(preProc,  x_hold[[imp]])
      preProc  <- preProcess(y_orig[[imp]], method=c("center", "scale")) #This ensures that the  hold out data is scaled using the tranformation used for the training set
      y_orig[[imp]] <- predict(preProc, y_orig[[imp]])
      y_hold[[imp]]  <- predict(preProc,  y_hold[[imp]])
      #Filter the data to include top correlations
      # df <- data.frame(names=character(),av_pval = double(),stringsAsFactors=FALSE) 
      # pval_lm <- data.frame(pval_lm=double())
      # for(x in 1:ncol(x_orig[[imp]])){
      #   data <- x_orig[[imp]][,x]
      #   for(y in 1:ncol(y_orig[[imp]])){
      #     fit = lm(data ~ y_orig[[imp]][,y])
      #     pval_lm[y,1] <-  anova(fit)$'Pr(>F)'[1] 
      #   }
      #   df[x,] <- c(colnames(half_nets)[x], min(pval_lm))
      # }
      # x_orig_filter[[imp]] <- x_orig[[imp]]
      # x_orig_filter[[imp]][,df$names[order(df$av_pval, decreasing = FALSE)][1:(ncol(x_orig[[imp]])-filter)]] <- rep(NA, nrow(x_orig[[imp]]))
      
      #Use correlation method to filter
      orig_cov <- apply(cov(x_orig[[imp]], y_orig[[imp]]), 1, max)
      x_orig_filter[[imp]] <- x_orig[[imp]]
      x_orig_filter[[imp]][,names(orig_cov[order(abs(orig_cov), decreasing = FALSE)])[1:(ncol(x_orig[[imp]])-filter)]] <- rep(NA, nrow(x_orig[[imp]]))
      
      orig_fit[[imp]] <- spls(x_orig_filter[[imp]], y_orig[[imp]],  keepX= select_x, mode = "canonical", ncomp =comp,  scale=TRUE)
      ...
      for (i in 1:nperm){
        ...
        if(imp ==1){
          ...
          sample_for_perm_hold[[i]] <- sample(nrow(x_hold[[imp]])) #to enseure same sample is pulled out for each imputed set
          
        }
        x_perm <- x_orig[[imp]][sample_for_perm[[i]],] #Permuted data
        x_perm_hold <- x_hold[[imp]][sample_for_perm_hold[[i]],] #Permuted data
        
        # df <- data.frame(names=character(),av_pval = double(),stringsAsFactors=FALSE) 
        # pval_lm <- data.frame(pval_lm=double())
        # for(x in 1:ncol(x_perm)){
        #   data <- x_perm[,x]
        #   for(y in 1:ncol(y_orig[[imp]])){
        #     fit = lm(data ~ y_orig[[imp]][,y])
        #     pval_lm[y,1] <-  anova(fit)$'Pr(>F)'[1] 
        ...
        #   df[x,] <- c(colnames(half_nets)[x], min(pval_lm))
        # }
        # x_perm_filter <- x_perm
        # x_perm_filter[,df$names[order(df$av_pval, decreasing = FALSE)][1:(ncol(x_orig[[imp]])-filter)]] <- rep(NA, nrow(x_perm))
        
        perm_cov <- apply(cov(x_perm, y_orig[[imp]]), 1, max)
        x_perm_filter <- x_perm
        x_perm_filter[,names(perm_cov[order(abs(perm_cov), decreasing = FALSE)])[1:(ncol(x_perm)-filter)]] <- rep(NA, nrow(x_perm))
        
        perm_fit[[imp]] <- spls(x_perm_filter, y_orig[[imp]], keepX= select_x,mode = "canonical", ncomp = ncomp, scale=TRUE)
        hold_fit$variates$X <- as.matrix(x_perm_hold)%*%as.matrix(perm_fit[[imp]]$loadings.star[[1]])
        hold_fit$variates$Y <- as.matrix(y_hold[[imp]])%*%as.matrix(perm_fit[[imp]]$loadings.star[[2]])
        if(i ==1){
          permuted_cov_LV[[imp]] <- diag(cov(hold_fit$variates$X, hold_fit$variates$Y)) #Covariance between LV = Singular Value
          permuted_cor_LV[[imp]] <- diag(cor(hold_fit$variates$X, hold_fit$variates$Y)) #Correlation between LV 
          
        }
        if(i >1){
          permuted_cov_LV[[imp]] <- rbind(permuted_cov_LV[[imp]],diag(cov(hold_fit$variates$X, hold_fit$variates$Y)))
          permuted_cor_LV[[imp]] <- rbind(permuted_cor_LV[[imp]],diag(cor(hold_fit$variates$X, hold_fit$variates$Y)))
        }
      }
      for(comp in 1:ncomp){
        if(comp ==1){
          pval[[imp]] <- (sum(permuted_cov_LV[[imp]][,comp]>hold_cov_LV[[imp]][comp])+1)/(nperm + 1)
          CI_upper[[imp]] <- data.frame(sort(permuted_cov_LV[[imp]][,comp], decreasing = TRUE))[n_CI,1]
        }
        else {
          pval[[imp]] <- rbind(pval[[imp]],(sum(permuted_cov_LV[[imp]][,comp]>hold_cov_LV[[imp]][comp])+1)/(nperm + 1))
    ...
    #Save results 
    fname <- paste(wd, '/' , A, '_', B,'_cv_', cv,  '_filter_', filter, '_pvals_CI_and_av_cov_LVs.csv', sep='' )
    ...
    fname <- paste(wd, '/' , A, '_', B,'_cv_', cv,  '_filter_', filter,'_pvals_each_imputation.csv', sep='' )
    ...
    fname <- paste(wd, '/' , A, '_', B,'_cv_', cv,  '_filter_', filter,'_exp_var.csv', sep='' )
    ...
    for(comp in 1:ncomp){
      ...
      fname <- paste(wd, '/' ,A, '_', B,'_cv_', cv,  '_filter_', filter,'_LV_', comp, '.csv', sep='' )
      write.csv(variate, file = fname)
    }
    # for(comp in 1:ncomp){
    #   fname <- paste(wd, '/' , A, '_', B,'_cv_', cv,  '_filter_', filter, '_component_', comp, '_selected_variables.csv', sep='' )
    ...
    ggplot(av_pval, aes(X1,X2, fill=significant)) + 
      ...
      guides(fill=FALSE)
    fname <- paste(wd, '/' , A, '_', B,'_cv_', cv,  '_filter_', filter,'_pvals.png', sep='' )
    ggsave(fname, width = 10, height = 10, units = "cm")
    ...
    ggplot(data.frame(av_orig_cov_LV), aes(av_orig_cov_LV)) + 
      ...
      theme_minimal(base_size=15)+
      theme(axis.text.x = element_text(size=10, hjust=1), axis.text.y = element_text(size=10, hjust=1))
    fname <- paste(wd, '/' , A, '_', B,'_cv_', cv,  '_filter_', filter,'_SVs.png', sep='' )
...
for(contrast in c(9)){
  ...
  for(comp in 1:ncomp){
    ...
    fname <- paste(wd,'/s_', A, '_', B,'_cv_', cv,  '_filter_', filter, '_', comp, '_boot_X_loadings.csv', sep='' )
    ...
    if(brain_vis == TRUE){
      ...
      fname <- paste(wd,'/s_', A, '_', B,'_cv_', cv,  '_filter_', filter, '_', comp, '_adj_reliable_loading.csv', sep='' )
      write.csv(adj, file = fname, row.names=FALSE)
      adj_bin <- sign(adj)
      adj_bin[adj_bin==1] <- 2
      adj_bin[adj_bin==-1] <- 1
      fname <- paste(wd,'/s_', A, '_', B,'_cv_', cv,  '_filter_', filter, '_', comp, '_adj_reliable_loading_bin.csv', sep='' )
      write.csv(adj_bin, file = fname, row.names=FALSE)
      adj_pos <- adj
      adj_pos[adj_pos<0] <- 0
      fname <- paste(wd,'/s_', A, '_', B,'_cv_', cv,  '_filter_', filter, '_', comp, '_adj_reliable_loading_positive.csv', sep='' )
      write.csv(adj_pos, file = fname, row.names=FALSE)
      adj_neg <- adj
      adj_neg[adj_neg>0] <- 0
      adj_neg <- abs(adj_neg)
      
      fname <- paste(wd,'/s_', A, '_', B,'_cv_', cv,  '_filter_', filter, '_', comp, '_adj_reliable_loading_negative.csv', sep='' )
    ...
    fname <- paste(wd,'/s_', A, '_', B,'_cv_', cv,  '_filter_', filter,'_', comp,'_X_loadings.png', sep='' )
    ggsave(fname, width = 15, height = 10, units = "cm")
    ...
    fname <- paste(wd,'/s_', A, '_', B,'_cv_', cv,  '_filter_', filter, '_', comp,'_boot_Y_loadings.csv', sep='' )
    ...
    fname <- paste(wd,'/s_', A, '_', B,'_cv_', cv,  '_filter_', filter, '_', comp,'_Y_loadings.png', sep='' )
    ggsave(fname, width = 15, height = 10, units = "cm")
    ...
    fname <- paste(wd,'/s_', A, '_', B,'_cv_', cv,  '_filter_', filter, '_', comp,'_X_and_Y_loadings.png', sep='' )
    ggsave(fname, width = 15, height = 10, units = "cm")
