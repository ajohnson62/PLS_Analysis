...
for(contrast in c(20)){
  for(filter in c(1000, ncol(half_nets))){
    ...
    for(select_x in keepX){
      for(imp in 1:5){
        ...
        ncomp <- 1
        orig_corr <- apply(cor(y_orig[[imp]], x_orig[[imp]]), 1, max)
        y_orig_filter[[imp]] <- y_orig[[imp]]
        y_orig_filter[[imp]][,names(orig_corr[order(abs(orig_corr), decreasing = FALSE)])[1:(ncol(y_orig[[imp]])-filter)]] <- rep(NA, nrow(y_orig[[imp]]))
        
        orig_fit[[imp]] <- spls(x_orig[[imp]], y_orig_filter[[imp]],  mode = "canonical", ncomp =ncomp, scale=TRUE, all.outputs=TRUE, max.iter = 1000,keepY=select_x)
        ...
        for (i in 1:nperm){
          ...
          perm_corr <- apply(cor(y_orig[[imp]],x_perm), 1, max)
          y_perm_filter[[imp]] <- y_orig[[imp]]
          y_perm_filter[[imp]][,names(perm_corr[order(abs(perm_corr), decreasing = FALSE)])[1:(ncol(y_orig[[imp]])-filter)]] <- rep(NA, nrow(y_orig[[imp]]))
          
          perm_fit[[imp]] <- spls(x_perm, y_perm_filter[[imp]],  mode = "canonical", ncomp = ncomp, scale=TRUE, max.iter = 1000,keepY=select_x)
          if(i ==1){
            ...
            perm_fit_orig <- spls(x_perm_orig, y_perm_filter[[1]],  mode = "canonical", ncomp = ncomp, scale=TRUE, max.iter = 1000,keepY=select_x)
            #Note, don't use procrustes at this point as we're comparing singular values an procrustes reduces these
